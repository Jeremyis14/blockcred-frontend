...
export default function PricingPage() {
  ...
  return (
    <>
      ...
      <main className="bg-white dark:bg-slate-950 dark:text-slate-100 mt-9">
        <section className="pt-16 pb-8">
          <div className="px-6 mx-auto max-w-7xl">
            <div className="max-w-3xl mx-auto text-center">
              <h1 className="text-4xl font-medium text-gray-900 dark:text-slate-100 sm:text-5xl">
              ...
              <div className="inline-flex items-center gap-3 p-1 mt-6 bg-gray-100 dark:bg-slate-800 rounded-full shadow-sm dark:shadow-none dark:border dark:border-slate-700">
                <button
                  onClick={() => setBilling("monthly")}
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition ${
                    billing === "monthly" ? "bg-white text-gray-900 shadow dark:bg-slate-900 dark:text-slate-100" : "text-gray-600 dark:text-slate-300"
                  }`}
                >
                  Monthly
                </button>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition ${
                    billing === "yearly" ? "bg-white text-gray-900 shadow dark:bg-slate-900 dark:text-slate-100" : "text-gray-600 dark:text-slate-300"
                  }`}
                >
            ...
            <div className="grid gap-6 mt-12 sm:grid-cols-2 lg:grid-cols-3">
              {tiers.map((t, i) => (
                <article
                  key={t.id}
                  className={`relative overflow-hidden p-6 bg-white dark:bg-slate-900/70 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm transform transition-all duration-300 hover:-translate-y-2 hover:shadow-lg`}
                ...
                >
                  ...
                  <h3 id={`tier-${t.id}`} className="text-lg font-medium text-gray-900 dark:text-slate-100">
                    {t.name}
                  </h3>
                  ...
                  <ul className="mt-6 space-y-2 text-sm text-gray-600 dark:text-slate-300">
                    {t.perks.map((p) => (
